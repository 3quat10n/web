document.addEventListener("DOMContentLoaded", function() {
  ...
  document.body.addEventListener('click', function() {
    ...
    const word1 = "Equation";
    let charIndex1 = 0;

    function typeWriter1() {
      textElement1.textContent = word1.substring(0, charIndex1);
      charIndex1++;
      if (charIndex1 <= word1.length) {
        setTimeout(typeWriter1, 200);
      } else {
        setTimeout(typeWriter2, 500); // Start next typing effect after a short pause
      }
    }
    typeWriter1();

    // Typing effect for the list of words
    const textElement2 = document.querySelector('.typing-effect');
